...
#cp -f Makefile_minipro2 padavan-ng/trunk/user/minipro//minipro-0.7.4/Makefile
...
sed -i '16a\\t$(ROMFSINST) -p +x /usr/bin/sysinfo' padavan-ng/trunk/user/optware/Makefile
#cp qos.sh padavan-ng/trunk/user/optware/qos.sh
#sed -i '17a\\t$(ROMFSINST) -p +x /usr/bin/qos.sh' padavan-ng/trunk/user/optware/Makefile
...
#sed -i '39s/.*/#define SQUASHFS_FILE_SIZE		256/' padavan-ng/trunk/user/scripts/dev_init.sh
...
#sed -i 's/# CONFIG_LESS is not set/CONFIG_LESS=y/' padavan-ng/trunk/configs/boards/busybox.config
