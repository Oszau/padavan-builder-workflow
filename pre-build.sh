...
sed -i '12s/.*/COPTS = $(CPUFLAGS) -O1 $(filter-out -O%, $(CFLAGS))/' padavan-ng/trunk/libs/libssl-1.1/Makefile
sed -i '12s/.*/COPTS = $(CPUFLAGS) -O1 $(filter-out -O%, $(CFLAGS))/' padavan-ng/trunk/libs/libssl-3.1/Makefile
sed -i '12s/.*/COPTS = $(CPUFLAGS) -O1 $(filter-out -O%, $(CFLAGS))/' padavan-ng/trunk/libs/libssl-3.3/Makefile
sed -i '244s/.*/HOSTCFLAGS = -Wall -Wmissing-prototypes -Wstrict-prototypes -O1 -fomit-frame-pointer -std=gnu89/' padavan-ng/trunk/linux-3.4.x/Makefile
sed -i '245s/.*/HOSTCXXFLAGS = -O1/' padavan-ng/trunk/linux-3.4.x/Makefile
sed -i '573s/.*/KBUILD_CFLAGS += -O1 -fno-reorder-blocks -fno-tree-ch/' padavan-ng/trunk/linux-3.4.x/Makefile
